...
void setup()
{
  ...
  // lets clear out those seen buffers first ..
  //memset(seen_macs,0,6*MAX_MACS);
  //memset(seen_names,0,32*MAX_MACS);

  //memset(seen_macs,0,sizeof(seen_macs));
  //memset(seen_names,0,sizeof(seen_names));

  //memset(seen_macs[6],0,6);
  ...
  for(int i=0;i<MAX_MACS;++i)
  {
    ...
    memset(seen_macs[i],0,6);
    memset(seen_names[i],0,32);
  }
  */
